...
-- Вставить столбец после указанного столбца.
ALTER TABLE table_name ADD column_name column_type AFTER column_name_2;
...
```sql
-- Удалить столбец. Можно без COLUMN.
ALTER TABLE table_name DROP COLUMN column_name;
ALTER TABLE table_name DROP column_name;

-- Удаление двух столбцов.
ALTER TABLE table_name DROP column_name DROP column_name_2;
```
- Удалить столбец.

```sql
-- Переименование столбца. (MySQL)
ALTER TABLE таблица CHANGE имя_столбца новое_имя_столбца ТИП ДАННЫХ;

-- Изменение типа данных столбца. (MySQL)
...
```sql
-- Создание ограничения для col_name, отключая проверки.
ALTER TABLE table_name WITH NOCHECK ADD CHECK (col_name > 10);

-- Создание ограничения с именем.
ALTER TABLE table_name ADD CONSTRAINT check_name CHECK (col_name > 10);

-- Добавление уникального ограничения на email.
ALTER TABLE employees ADD CONSTRAINT unique_email UNIQUE (email);

-- Удаление ограничения.
ALTER TABLE table_name DROP check_name;
```
...
**CTE (Common Table Expressions, Обобщенное табличное выражение)** - это временный результирующий набор данных, к которому можно обращаться в последующих запросах.

CTE полезны для организации сложных запросов, улучшения читаемости кода и многократного использования промежуточных результатов. После определения CTE можно использовать его в основной части запроса так, как если бы это была отдельная таблица. Выражение с WITH считается временным, потому что результат не сохраняется где-либо на постоянной основе в схеме БД, а действует как временное представление, которое существует только на время выполнения запроса.
...
FROM имя_cte;
```
- Пример базового синтаксиса. CTE обычно создается с помощью ключевого слова `WITH` перед основным запросом.
...
```
- Предположим, есть таблица `employees` с колонками `id`, `name`, `department`, `salary`. Мы хотим найти среднюю зарплату в каждом департаменте и сотрудников, зарабатывающих больше среднего.
- Мы создаем CTE `AverageSalary`, где рассчитываем среднюю зарплату в каждом департаменте.
- В основном запросе мы соединяем таблицу `employees` с `AverageSalary` и отбираем сотрудников, зарабатывающих выше среднего.
...
**Оконные функции** - это функции, которые применяются к набору строк в пределах определенного окна, заданного в запросе, и возвращают результат для каждой строки этого окна. Это позволяет выполнять вычисления по группе строк без группировки их в одну строку (В отличие от агрегатных функций).
...
В стандартном SQL-запросе все наборы строк рассматриваются как один сплошной блок данных, для которого и вычисляются агрегатные значения. Однако, когда применяются оконные функции, запрос сегментируется на группы строк (или окна), и для каждого такого сегмента подсчитываются индивидуальные агрегатные значения.
...
- Далее следует OVER, который определяет окно, которое будет передаваться в оконную функцию. Если OVER оставить без параметров, то окном будет выступать вся таблица.
...
При использовании `ROWS`, определение окна основывается на физическом положении строк относительно текущей строки. Например, `1 PRECEDING` означает одну строку до текущей. `RANGE`, в отличие от `ROWS`, определяет границы окна на основе значений столбцов, упорядоченных в соответствии с ORDER BY в оконной функции. Определение окна с помощью `ROWS` чётко ограничивает количество строк, которые включаются в окно, делая его предсказуемым и конкретным. Границы, определённые с помощью `RANGE`, могут варьироваться в зависимости от данных, что делает окно гибким, но потенциально менее предсказуемым.
...
**Ранжирующие оконные функции** - это функции, которые ранжируют значение для каждой строки в окне. В ранжирующих функциях под ключевым словом OVER обязательным идёт указание условия ORDER BY, по которому будет происходить сортировка ранжирования.

- `ROW_NUMBER`: Возвращает номер строки, используется для нумерации.
- `RANK`: Возвращает ранг каждой строки.
- `DENSE_RANK`: В отличие от функции RANK, она для одинаковых значений возвращает ранг, не пропуская следующий.
