...
**CI/CD (Continuous Integration / Continuous Delivery|Deployment)** - это непрерывная интеграция и непрерывное развертывание ПО в процессе разработки.

**Непрерывная интеграция** - это процесс разработки ПО, при котором код автоматически собирается, тестируется и интегрируется в репозиторий с целью минимизации разрыва между компонентами проекта и быстрой обратной связи о качестве кода.

**Непрерывная доставка** - это процесс разработки ПО, при котором продукт находится в готовом состоянии и может быть передан в промышленную эксплуатацию.

**Непрерывное развертывание** - это процесс разработки ПО, при котором обновления продукта регулярно вводятся в промышленную эксплуатацию, например, после каждого изменения. При этом все процессы предметной области продолжают работать без сбоев.

![](/materials/images/devops/devops_whatis_ci_cd.png)

**Runner** - это приложение, в рамках которого выполняются задачи и которое можно развернуть на разных типах систем.

**Jobs** - это элементы из которых строится процесс CI/CD. Может включать сборку проекта, автотестирование, публикация проекта. По умолчанию задачи выполняются изолированно, но их можно связать с помощью артефактов.

**Artifacts** - это исполняемые файлы для передачи результатов выполнения одной задачи на вход другой. Это позволяет управлять жизненным циклом ПО. Это могут быть скомпилированные бинарные файлы, архивы, образы контейнеров.

**Stages** - это элементы, которые группируют задачи и определяют порядок их выполнения. Задачи, которые принадлежат одному этапу, выполняются параллельно, если доступно нужное кол-во раннеров. Этапы выполняются в порядке, указанном в конфиге.

**Pipeline** - это верхнеуровневый элемент процесса CI/CDБ включающий этапы и задачи.

Популярные инструменты CI/CD: CircleCI, Github Actions, Gitlab CI, Drone, Travis CI, Jenkins, TeamCity, Azure DevOps.

## Принцип работы CI/CD

Описание принципа работы на примере Gitlab CI.

- Разработчик пишет код и заливает его в репозиторий проекта.
- Gitlab ищет в корне репозитория конфиг `.gitlab-ci.yml` и запускает пайплайн согласно описанной в конфиге логике.
- Пайплайн представляет собой процесс из этапов (stage), которые состоят из задач (job). Каждая задача выполняется в изолированном процессе (Gitlab Runner).
