...
Разделение привилегий - одна из основных парадигм безопасности в ОС семейства Linux и Unix. Специальный пользователь с именем root, имеет привилегии суперпользователя. Это административная учетная запись без ограничений, действующих для обычных пользователей. Пользователи могут выполнять команды с привилегиями суперпользователя или **root** разными способами.

Постоянно использовать учётную запись root не рекомендуется, поскольку, имея абсолютные права доступа, можно совершенно случайно нанести системе непоправимый вред. Поэтому в системе существует команда su (substitute user), позволяющая обычному пользователю получать root-права в любой момент.
...
su
```
- Получение root-прав. Система запросит пароль root-пользователя, после чего откроет доступ к сессии оболочки root-пользователя.

```bash
sudo command
```
- Другой способ получения привилегий root. Команда sudo позволяет выполнять отдельные команды с правами root без необходимости открывать новую сессию. В отличие от su, команда sudo запрашивает не пароль root, а пароль пользователя, который вызывает команду.

По умолчанию sudo запоминает пароль в течение 15 минут после его ввода. Вот почему нужно ввести пароль только один раз при быстром выполнении нескольких команд с помощью sudo.

```bash
sudo -k
```
- Использование sudo, чтобы sudo запрашивал пароль при следующем запуске, а не запоминал.
...
visudo
```
- Открыть файл `/etc/sudoers` для редактирования. При сохранении файла он будет проверен на соответствие установленному формату (в противном случае отменит изменения).
...
- `Defaults env_reset` сбрасывает среду терминала для удаления переменных пользователя. Эта мера безопасности используется для сброса потенциально опасных переменных среды в сеансе `sudo`.
- `Defaults mail_badpass` предписывает системе отправлять уведомления о неудачных попытках ввода пароля `sudo` для настроенного пользователя `mailto`. По умолчанию это учетная запись `root`.
- `Defaults secure_path=…` задает переменную `PATH` (места в файловой системе, где ОС будет искать приложения), которая будет использоваться для операций `sudo`. Это предотвращает использование пользовательских путей, которые могут быть вредоносными.
- `root ALL=(ALL:ALL) ALL` определяет для пользователя `root` привилегии `sudo`. Первое `ALL` означает, что данное правило применяется ко всем хостам. Второе `ALL` означает, что пользователь root может запускать команды от лица всех пользователей. Третье `ALL` означает, что пользователь root может запускать команды от лица всех групп. Последнее `ALL` означает, что данные правила применяются всем командам. Таким образом, это означает, что пользователь root сможет выполнять любые команды с помощью `sudo` после ввода пароля.

```bash
defaults timestamp_timeout = 0
```
- Отключение запоминания пароля. Необходимо добавить под другими строками defaults.

Также, можно изменить время ожидания пароля, включить функцию “никогда не спрашивать пароль”, настроить запуск определенных команд без пароля, разрешить пользователю запускать только определенные команды, сделать логирование доступа к sudo и т.д.
