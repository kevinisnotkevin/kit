...
systemctl - это утилита systemd для управления службами в linux.
...
```
- Запуск сервиса app. `.service` указывает на то, что это служба, однако systemd позволяет это не указывать при работе с утилитами по управлению службами.
...
```
- Перезагрузка файлов конфигурации без перезапуска (если поддерживается).

```bash
systemctl reload-or-restart app
```
- Перезагрузка необходимой конфигурации при наличии, иначе перезагрузка службы.

```bash
systemctl enable app
```
- Включение автоматического запуска службы при загрузке. При этом будет создана символическая ссылка из системной копии служебного файла (обычно в `/lib/systemd/system` или `/etc/systemd/system`) в месте на диске, где `systemd` ищет файлы для автозапуска.

```bash
systemctl disable app
```
- Отключение автоматического запуска службы. При этом будет удалена символическая ссылка, что укажет на то, что служба не должна запускаться автоматически.

```bash
systemctl status app.service
```
- Вывод статуса службы, иерархии контрольных групп и первых несколько строк журнала.
...
systemctl
```
- Вывод списка всех активных сервисов (Поведение по умолчанию). Вывод содержит следующие столбцы:
...
**service** - это утилита, которая содержится в утилите systemctl и предоставляет те же возможности мониторинга служб linux.

```bash
service --status-all
```
- Вывод списка всех запущенных сервисов.
