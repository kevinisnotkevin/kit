...
- `base/`: Основная директория для хранения данных таблиц и индексов. Каждая БД в системе имеет свою поддиректорию.
- `pg_wal/`: Директория для хранения журналов предзаписи (Write-Agead Logs). Обеспечивает устойчивость данных в случае сбоя.
- `pg_stat/`: Директория для хранения статистики, собранной системой для работы с запросами и индексами.
- `pg_tblspc/`: Директория для хранения символьных ссылок на таблицы или БД, которые могут находиться в других каталогах или на других дисках.
- `pg_clog/`: Директория для хранения информации о транзакциях и их состоянии.

**Конфигурационные файлы**

- `postgresql.conf`: Основной конфигурационный файл, в котором настраиваются параметры работы сервера.
- `pg_hba.conf`: Файл определяет, какие пользователи и какие хосты могут подключаться к базе данных, а также какие способы аутентификации используются.
- `pg_ident.conf`: Файл, используемый для маппинга пользователей ОС на пользователей PostgreSQL при определенных способах аутентификации.
...
По умолчанию конфигурация PostgreSQL не настроена для рабочей нагрузки. Значения по умолчанию установлены для обеспечения работоспособности PostgreSQL везде с наименьшим количеством ресурсов. Имеются настройки по умолчанию для всех параметров базы данных.

## Настройка master-slave репликации

1. Устанавливаем БД одной версии на обоих серверах
2. Включаем БД на обоих серверах
3. Настраиваем конфиг master сервера
4. Перезапускаем master сервер
5. Подключаемся к БД master сервера и создаем пользователя с правами для выполнения репликации
6. Настраиваем конфиг slave сервера
7. Выключаем БД на slave сервере и удаляем содержимое директории с данными БД
8. Запускаем репликацию на slave сервере
9. Включаем БД на slave сервере
10. Проверяем статус репликации
